...
def loadProject(proj) :
    ...
    if not os.path.isfile(dir_path + '\\projects\\' + proj) :
        with open(dir_path + '\\projects\\' + proj, 'wb') as file :
            pickle.dump(newProject(), file)
    return pickle.load(open(dir_path + '\\projects\\' + proj, 'rb'))

def newProject() :
    return {"programmingSymbols" : ['|', '{', '}', '[', ']','=', ';', "_", "/", "-", "!", "\'", "\""],
    "searchStrings" : ['additive manfacturing', 'Service logistics 3D printing',\
     'consolidation 3D printing'],
    "categories" : ["News", "Web"],
    "mboxPaths" : ["Google Alerts1.mbox", "Google Alerts2.mbox", "Google Alerts3.mbox"],
